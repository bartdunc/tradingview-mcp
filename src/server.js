...
const server = new McpServer(
  ...
  {
    instructions: `TradingView MCP — 81 tools for reading and controlling a live TradingView Desktop chart via CDP on localhost:9222.

TOOL SELECTION GUIDE — use this to pick the right tool:

Reading your chart:
- chart_get_state → get symbol, timeframe, all indicator names + entity IDs (call first, once)
...
Daily session bias (morning brief workflow):
- morning_brief → scans rules.json watchlist, sets each symbol, reads indicators + quote, restores chart, returns structured data + bias_criteria. YOU apply the criteria and emit the bias text.
- session_save → persists the brief to ~/.tradingview-mcp/sessions/YYYY-MM-DD.json
- session_get → today's brief, or yesterday's if today isn't saved
- rules.json must live in <project>/ or ~/.tradingview-mcp/ — other paths are rejected
...
- chart_set_visible_range → zoom to exact unix-timestamp range
- indicator_set_inputs / indicator_toggle_visibility → tune or hide a study
...
- pine_analyze → OFFLINE static checks (no chart needed). pine_check → server-side compile check.
...
Panes: pane_list, pane_set_layout (s, 2h, 2v, 2x2, 4, 6, 8), pane_focus, pane_set_symbol
Tabs: tab_list, tab_new, tab_close, tab_switch
Watchlist: watchlist_get, watchlist_add
Health: tv_health_check, tv_discover (probe known TV paths), tv_ui_state, tv_launch
...
- Call chart_get_state ONCE at start, reuse entity IDs (they are session-specific)`,
  },
);
